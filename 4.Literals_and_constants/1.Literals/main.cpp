...
int main()
{
    ...
    unsigned char unsigned_char{53u}; // 555u will fail because of narrowing
    ...
    unsigned short int unsigned_short_int{5678u}; // No special literal type for short
    ...
    // With number system - Hex : prefix with 0x
    unsigned int hex_number{0x22BU}; // Dec 555
    int hex_number2{0x400};          // Dec 1024
    std::cout << std::hex << "hex_number is : " << hex_number << std::endl;
    std::cout << std::dec << "hex_number2 is : " << hex_number2 << std::endl;

    // Representing colors with hex
    int black_color{0xffffff};
    std::cout << std::dec << "Black color is : " << black_color << std::endl;
